  ...
  return <aside>
    <ul>
      <li className="category">
        <button className="btn btn-all-categories btn-sq" onClick={() => setCurrentCategory("All")}>All</button>
      </li>
      {CATEGORIES.map((cat) => (
        <li key={cat.name} className="category">
          <button className="btn btn-category btn-sq" onClick={() => setCurrentCategory(cat.name)}>
            {cat.name}
          </button>
        </li>
      ))}
...
function Fact({fact, setFacts}) {
  ...
  async function handleVote(columnName) {
    setIsUpdating(true);
    const {data: updateFact, error} = await supabase.from('facts').update({[columnName]: fact[columnName]+1}).eq("id", fact.id).select();
    setIsUpdating(false);
